...
template<typename T>
std::unique_ptr<ValueComparison<T> > in_range(const T& low, const T& high, bool end_inclusive = false,
...
template<typename T>
std::unique_ptr<ValueComparison<Facet> > in_rangef(const T& low, const T& high, bool end_inclusive = false,
                                                   bool start_inclusive = true) {
    return in_range(fcast(low), fcast(high), end_inclusive, start_inclusive);
}

template<typename T>
std::unique_ptr<ValueComparison<Facet>> approx_in_rangef(const T& low, const T& high
