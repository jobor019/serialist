...
public:
    ...
    static const Mode DEFAULT_MODE = Mode::from_bottom;
    static constexpr auto DEFAULT_STRATEGY = Index::Strategy::mod;
    static const bool DEFAULT_CURSOR_IS_INDEX = true;
...
private:
    Voice<T> select(const Voice<T>& v
        ...
        auto result = Voice<T>::allocated();
        for (const Index& i : indices) {
            result.append(select_single(v, i, octave, strategy, invert));
        }
    ...
    Voice<T> random(const Voice<T>& v, std::optional<Index> count) {
        ...
        return m_rnd.choices(v, count, true);
    }


    const T& select_single(const Voice<T>& v
                           , const Index& index
                           , std::optional<T> octave
                           , Index::Strategy strategy
                           , bool invert) {
        ...
        return v[index.get(v.size(), invert)];
    }
...
public:
    ...
    PatternizerNode(const std::string& id
        ...
        , m_trigger(NodeBase<T>::add_socket(Keys::TRIGGER, trigger))
    ...
    Voices<T> process() {
        ...
        auto num_voices = NodeBase<T>::voice_count(trigger, chord, cursor, mode, octave);
        ...
        auto modes = mode.firsts_or(Patternizer<T>::DEFAULT_MODE).adapted_to(num_voices);
        auto octaves = octave.firsts().adapted_to(num_voices);
        ...
        for (size_t i = 0; i < num_voices; ++i) {
            if (Trigger::contains_pulse_on(triggers[i])) {
                if (is_index) {
                    m_current_value[i] = Patternizer<T>::process(
                        chords[i]
                        , cursors[i].template as_type<Index>(&Index::from_index_facet)
                        , modes[i]
                        , octaves[i]
                    );
                } else {
...
struct PatternizerWrapper {
    ...
    Sequence<Facet, StoredType> octave{Keys::OCTAVE, ph};
    ...
    PatternizerT patternizer_mode{Keys::CLASS_NAME
                                  , ph
                                  , &trigger
                                  , &chord
                                  , &cursor
