...
public:
    ...
    virtual std::vector<T> get(double position, const Mapping<T>& mapping) = 0;
...
protected:
    std::size_t get_index(double position, std::size_t map_size) {
        ...
        return static_cast<std::size_t>(std::floor(position * static_cast<double>(map_size) + std::copysign(1e-8, position)));
    }
