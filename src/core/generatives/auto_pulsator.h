...
public:
    ...
    void update_parameters(std::size_t num_voices, bool size_has_changed) override {
            ...
            m_pulsators.set(&AutoPulsator::set_duration, duration.as_type<double>());
        }

        if (size_has_changed || m_legato_amount.has_changed()) {
            auto legato = m_legato_amount.process().adapted_to(num_voices).firsts_or(4.0);
            m_pulsators.set(&AutoPulsator::set_legato_amount, legato.as_type<double>());
        }
    }

private:
    MultiVoiced<AutoPulsator, Trigger> m_pulsators;
